...
/**
 * Custom pages entity for report builder.
 *
...
/**
 * Report builder entity for custom pages.
 *
 * @package    local_custompage
 */
class custompages extends base {
    ...
    protected function get_all_columns(): array {
        ...
        $columns[] = (new column(
            'sortthread',
            new lang_string('sortorder', 'core'),
            $this->get_entity_name()
        ))
        ->add_joins($this->get_joins())
        ->set_type(column::TYPE_TEXT)
        ...
        $columns[] = (new column(
            'iscontainer',
            new lang_string('iscontainer', 'local_custompage'),
            $this->get_entity_name()
        ))
            ->add_joins($this->get_joins())
        ...
        $columns[] = (new column(
            'parent',
            new lang_string('parentpage', 'local_custompage'),
            $this->get_entity_name()
        ))
            ->add_joins($this->get_joins())
            ->set_type(column::TYPE_TEXT)
            ...
            ->add_callback(static function (?string $value, \stdClass $page): string {
                if (empty($page->parent)) {
                    return get_string('noparent', 'local_custompage');
                }
                ...
                try {
                    ...
                    return get_string('invalidparent', 'local_custompage');
                }
            });
    ...
    protected function get_all_filters(): array {
        ...
        ->set_options_callback(static function (): array {
            global $DB;
            $pagenames = $DB->get_records_sql('SELECT DISTINCT id,name,contextid FROM {local_custompages} ORDER BY name ASC');
