    ...
    public static function execute(array $pageids, int $parentid = 0): array {
        ...
        foreach ($pageids as $pageid) {
            ...
            if ($actualparent != $parentid) {
                throw new moodle_exception('invalidparent', 'local_custompage');
            }
        ...
        try {
            ...
            return [
                'success' => true,
                'message' => get_string('sortorderupdated', 'local_custompage'),
            ];
        } catch (\Exception $e) {
            $transaction->rollback($e);
            throw new moodle_exception('errorsortingpages', 'local_custompage', '', null, $e->getMessage());
        }
    }
