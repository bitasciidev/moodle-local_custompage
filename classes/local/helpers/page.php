...
class page {
    ...
    public static function get_page_tree(?int $parentid = null): array {
        ...
        $pages = $DB->get_records('local_custompages', ['parent' => $parentid], 'sortthread ASC');
        $tree = [];
    ...
    public static function get_breadcrumb(page_model $page): array {
        ...
        foreach ($ancestors as $ancestor) {
            ...
            'url' => new moodle_url('/local/custompage/view.php', ['id' => $ancestor->get('id')]),
            ];
        }

        $breadcrumb[] = [
          'name' => $page->get_formatted_name(),
          'url' => new moodle_url('/local/custompage/view.php', ['id' => $page->get('id')]),
        ];
    ...
    public static function delete_page(int $pageid): bool {
        ...
        try {
            ...
            $children = $DB->get_records('local_custompages', ['parent' => $pageid]);
            foreach ($children as $child) {
                $DB->set_field('local_custompages', 'parent', null, ['id' => $child->id]);
