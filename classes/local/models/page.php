...
use Exception;
use lang_string;
use local_custompage\constants;
...
use local_custompage\local\helpers\vancode;
 ...
 *
 * This class represents a custom page stored in the local_custompages table.
 *
 * LIFECYCLE HOOKS:
 ...
 *
 * IMPORTANT: For page deletion, use \local_custompage\local\helpers\page::delete_page()
...
class page extends persistent {
    ...
    public function swap_item_sortthreads(int $itemid1, int $itemid2): bool {
        ...
        $items = $DB->get_records_select('local_custompages', "id $insql", $inparams);
        ...
        if ($DB->record_exists_select('local_custompages', $DB->sql_like('sortthread', '?'), ['%swaptemp%'])) {
            return false;
        }
    ...
    public function get_url(): moodle_url {
        return new moodle_url('/local/custompage/view.php', ['id' => $this->get('id')]);
    }
    ...
    protected function after_create(): void {
        ...
        custompage_created::create_from_object($this)->trigger();
    }
     ...
     * \local_custompage\local\helpers\page::delete_page() for explicit control.
    ...
    public function update_path(): void {
        ...
        $DB->set_field('local_custompages', 'path', $path, ['id' => $this->get('id')]);
        $DB->set_field('local_custompages', 'depth', $this->raw_get('depth'), ['id' => $this->get('id')]);
    }
    ...
    public function get_children(): array {
        ...
        $records = $DB->get_records('local_custompages', ['parent' => $this->get('id')], 'sortthread ASC');
        $children = [];
    ...
    private function generate_sortthread(): void {
        ...
        $DB->set_field('local_custompages', 'sortthread', $newsortthread, ['id' => $this->get('id')]);
    }
    ...
    private function update_moved_page_sortthread(): void {
        ...
        $existingsortthreads = [];
        if ($parentid) {
            $siblings = $DB->get_records('local_custompages', ['parent' => $parentid], '', 'id, sortthread');
        } else {
            $siblings = $DB->get_records_select('local_custompages', 'parent IS NULL OR parent = 0', [], '', 'id, sortthread');
        }
    ...
    public function move_to_parent(?int $newparentid): void {
        ...
        try {
            ...
            $DB->set_field('local_custompages', 'depth', $newdepth, ['id' => $id]);
            $DB->set_field('local_custompages', 'path', $newpath, ['id' => $id]);
            ...
            $descendants = $DB->get_records_select('local_custompages', $sql, ['like' => $likeparam], '', 'id, path, depth');
            ...
            foreach ($descendants as $desc) {
                ...
                $DB->set_field('local_custompages', 'path', $newchildpath, ['id' => $desc->id]);
                $DB->set_field('local_custompages', 'depth', ((int)$desc->depth) + $deltadepth, ['id' => $desc->id]);
            }
    ...
    private function update_children_sortthreads(int $parentid, string $parentsortthread): void {
        ...
        $children = $DB->get_records('local_custompages', ['parent' => $parentid], 'sortthread ASC');
        $childcounter = 1;
    ...
    protected function validate_parent(?int $parentid): lang_string|bool {
        ...
        if ($parentid == $this->get('id')) {
            return new lang_string('cannotbeselfparent', 'local_custompage');
        }
        ...
        try {
            ...
            if (!$parent->is_container()) {
                return new lang_string('parentmustbecontainer', 'local_custompage');
            }

            if ($parent->get('depth') >= constants::PAGE_MAX_DEPTH) {
                return new lang_string('maxdepthreached', 'local_custompage');
            }

            if ($this->get('id') && $parent->is_descendant_of($this)) {
                return new lang_string('circularreference', 'local_custompage');
            }
        } catch (Exception $e) {
            return new lang_string('invalidparent', 'local_custompage');
        }
    ...
    /**
     * Custom validation for iscontainer field
     *
     * @param int $value
     * @return bool|lang_string
     * @throws coding_exception
    ...
    protected function validate_iscontainer(int $value): lang_string|bool {
        ...
        if ($this->get('id') > 0 && $value == 0) {
            if ($this->has_children()) {
                return new lang_string('containerhaschildren', 'local_custompage');
            }
        }

        // If setting as container and has parent, container pages must be root level.
        if ($value == 1 && $this->get('parent')) {
            return new lang_string('containermustberoot', 'local_custompage');
        }
    ...
    public function has_children(): bool {
        global $DB;
        return $DB->record_exists('local_custompages', ['parent' => $this->get('id')]);
    }
    ...
    private function update_all_sortthreads(): void {
        ...
        $existingsortthreads = $DB->get_fieldset_select('local_custompages', 'sortthread', 'sortthread IS NOT NULL');
        ...
        foreach ($rootpages as $rootpage) {
            ...
            $DB->set_field('local_custompages', 'sortthread', $newsortthread, ['id' => $rootpage->id]);
